...
, str(photo_data_url or ''), re.S)
    ...
    if len(raw) > 10 * 1024 * 1024:
        raise ValueError('telegram_photo_too_large')
    mime = match.group(1)
    filename = 'asas-field-photo.' + ('jpg' if mime == 'image/jpeg' else mime.split('/')[1])
    boundary = '----ASAS' + secrets.token_hex(12)
    parts = []
    ...
    body = b''.join(parts)
    request = urllib.request.Request(
        'https://api.telegram.org/bot{}/sendPhoto'.format(token), data=body,
