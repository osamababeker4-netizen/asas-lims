...
EXPECTED_VERSION='10.3.0-decision-intelligence'
...
def health_check():
    status, headers, body=once(urllib.request.Request(API+'/api/health', headers={'User-Agent':'ASAS-LIMS-Acceptance/10.3.0'}))
...
preflight=urllib.request.Request(
    ...
    headers={
        ...
        'User-Agent':'ASAS-LIMS-Acceptance/10.3.0'
    }
)
...
if STRICT_PRODUCTION:
    def pages_check():
        status, headers, body=once(urllib.request.Request(PAGES, headers={'Cache-Control':'no-cache','User-Agent':'ASAS-LIMS-Acceptance/10.3.0'}))
        text=body.decode('utf-8','replace')
        if status == 200 and 'V10.3.0 Decision Intelligence' in text and '10-3-0-decision-intelligence' in text:
            return 'current'
        return None
