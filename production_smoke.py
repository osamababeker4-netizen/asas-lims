...
EXPECTED_VERSION='10.2.23-field-guide-proxy'
...
def health_check():
    status, headers, body=once(urllib.request.Request(API+'/api/health', headers={'User-Agent':'ASAS-LIMS-Acceptance/10.2.23'}))
...
preflight=urllib.request.Request(
    ...
    headers={
        ...
        'User-Agent':'ASAS-LIMS-Acceptance/10.2.23'
    }
)
...
if STRICT_PRODUCTION:
    def pages_check():
        status, headers, body=once(urllib.request.Request(PAGES, headers={'Cache-Control':'no-cache','User-Agent':'ASAS-LIMS-Acceptance/10.2.23'}))
        text=body.decode('utf-8','replace')
        if status == 200 and 'V10.2.23 Field Guide' in text and '10-2-23-field-guide-proxy' in text:
            return 'current'
        return None
